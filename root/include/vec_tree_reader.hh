...
#ifndef ROOT__vec_tree_reader_HH
#define ROOT__vec_tree_reader_HH 
...
//  Tree reader class
//-----------------------------------------------------------------------------
class vec_tree_reader
{
public:
    vec_tree_reader(std::string filename = "", std::string tree_name = "");

    vec_tree_reader(const std::vector<std::string>& files, 
  ...
  */
    ...
    bool entry_in_range(double branch_at_jetn, std::string name);
...
//  Element Access
//-----------------------------------------------------------------------------
    std::vector<std::vector<double> > at(const unsigned int &idx);
    std::vector<std::vector<double> > operator[](const unsigned int &idx);
    std::vector<std::vector<double> > operator()(const unsigned int &idx);
    std::vector<double> operator()(const unsigned int &idx, std::string col_name);
    std::map<std::string, std::vector<double> > at(const unsigned int &idx, 
        const std::vector<std::string> &names);
    std::map<std::string, std::vector<double> > operator()(const unsigned int &idx, 
        const std::vector<std::string> &names);
...
private:
    ...
    typedef __INTERNAL::vector_handler number_container;
...
/*
template <class T>
agile::dataframe vec_tree_reader::get_dataframe(T &weights, int entries, int start, 
    bool verbose)
{
    ...
    if (verbose)
    {
        std::cout << "\nPulling agile::dataframe from vec_tree_reader..." << std::endl;
    }
